...
class Exp_ended_posts_category_tmb_1 extends WP_Widget
{
    function __construct()
    {
        parent::__construct('Exp_ended_posts_category_tmb_1', '(ویدیو)سایدبار با تصویر شاخص', array('description' => 'آخرین مطالب منتشر شده دسته ها'));
    }
    function widget($args, $instance)
    {
        ...
        <div class="sidebar-img">
            ...
            <?php
            if(have_posts()):
                if($instance['cat'] != "none"){
                    $query = new WP_Query(array(
                        'posts_per_page' => $instance['count1'],
                        'post_type' => 'video',
                        'tax_query' => array(
                            array(
                                'taxonomy' => 'cat-video',   // taxonomy name
                                'terms' =>  $instance['cat'],                  // term id, term slug or term name
                            )
                        )
                    ));
                }else{
    ...
    function form($instance)
    {
        ...
        <?php
        $object_tax = get_object_taxonomies('video', 'names');
        echo '<input type="hidden" value="' . $object_tax[1] . '" name="' . $this->get_field_name('taxonomy') . '">';
        ?>
        <p>
...
function myplugin_register_widgets_ended_posts_category_tmb_1()
{
    register_widget('Exp_ended_posts_category_tmb_1');
}

add_action('widgets_init', 'myplugin_register_widgets_ended_posts_category_tmb_1');